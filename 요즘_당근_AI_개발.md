...
## QA1. 개발자 없이 AI 프로덕트 a-z 완성하기

> [해당 영상](https://www.youtube.com/watch?v=TqCTf9L7q9U) / 날짜: 2025. 10. 13.

바이브 코딩 & Claude 활용 팁

- 코딩하다가 중간중간 계속 물어보는 게 좋다. (부록 A 참고!)
    - "지금 짠 코드에서 비효율적인 부분 찾아줘."
    - "더 개선할 수 있는 부분 있어? (성능, 가독성, 유지보수 측면에서)"
    - "보안상 문제 될 만한 코드 있는지 점검해줘."
- **맥락 설명**이 정말 중요함. 예시 2~3개 넣어주고, 역할극 시키는 것도 좋다.
- LLM을 '테스트 처리 에이전트'로 만들 때 프롬프트 순서
  - "너는 [누구]고, [무슨 업무]를 하며, [사실] 기반으로 답해야 해." (역할 부여)
  - [상세한 맥락] (TMI처럼 느껴져도 다 알려주는 게 좋음)
  - [예시 2~3개]

중요한 건 '기술'이 아니라 '`문제 정의`’

- 단순히 시간 많이 드는 업무를 자동화하는 것도 좋지만, 그게 본질은 아니다.
- 진짜 중요한 점은
    - 문제를 정확히 정의해야 하고,
    - 그 문제에 딱 맞는 기술을 찾아서 녹여내는 것이다.
- 결국, "내가 속한 도메인에서 어떤 문제를 풀어야 하는가"를 아는 역량이 핵심이다.
- 팁: 자동화하려는 작업을 잘게 쪼갠 다음, 이 쪼갠 개별 단위들을 하나씩 연결하면 결과물이 괜찮게 나온다.

---

## QA2. CS 도메인에서 AI로 압도적인 사용자 경험 제공하기

> [해당 영상](https://www.youtube.com/watch?v=7eE0gHNOEmA) / 날짜: 2025. 10. 27

### VoC 플레이그라운드

고객의 소리(정성적인 데이터) → 정량 데이터

- 기존 문제: 정성적인 고객 피드백은 정량적인 지표로 바꾸기 어려웠음
- AI 활용하여 분석 시간을 단축하고 신뢰도를 확보함.
- 신뢰도 확보하는 방법은
    - AI 가 “요즘 알림이 잘 안온다”는 문제점을 요약하면,
    - 실제 관련 문의 원본(원촌 소스)을 바라 볼 수 있게 링크함.

### 당근의 모든 사용자에게 AI 에이전트 제공하기

- LLM (e.g. GPT-4): 그냥 똑똑한 '모델' 자체.
    - 한계: 최신 정보 모르고, 우리 회사 도메인 지식은 약함.
- AI 에이전트 (Agent): LLM(뇌) + 프롬프트(지시) + 도구(손발)
    - 에이전트가 `도구`를 사용해서 LLM의 한계(최신 정보, 내부 데이터)를 극복함.
- 차이점이라면
    - LLM → 내 업무의 '일부'를 자동화하도록 도와줌.
    - AI 에이전트 → 목표만 주면, 알아서 자율적으로 달성함. (나를 '대체'할 수 있는 고차원적인 존재)

- 전세계적으로도 AI Agent를 잘활용하고 있는 도메인도 `CS` 도메인 쪽이다.
    - 문의 & 답변 측이 가장 크다.
    - 뒷단에도 활용하고 있다. → 신고 처리, 거래 금지 품목

AI 에이전트, 왜 CS 도메인인가?

- 전 세계적으로 AI 에이전트가 가장 활발하게 쓰이는 곳이 `CS 도메인` 이다.
- 과거에는 시나리오 기반의 챗봇이였다면, 현재는 “알잘딱깔센”의 AI 에이전트로 바뀌었다.
- 활용처는
    - 고객 앞단: 문의/답변
    - 백엔드(운영): 신고 처리, 거래 금지 품목 모니터링

왜 AI 에이전트는 멀티가 필요한가?

- 혼자 일 잘하는 에이전트도 좋지만, 팀으로 함께 일하는게 훨씬 효율이 크고 큰 임팩트를 낸다.
- 멀티 에이전트가 필요한 기준: 도구의 개수
    - 한 에이전트가 가진 도구(API, 벡터 서치, 웹 서치 등)가 **5개가 넘어가면** 도메인 별로 쪼개서 멀티 에이전트로 구성하는 것을 고려한다.

모델 선택 기준 & 평가

- 모델 선택
    - `GPT-4.1`: 지시사항을 잘 따름.
    - `GPT-5`: 자아가 강해서 프롬프트를 무시하기도 함.
    - `소넷(Claude 3 Sonnet)`: 도구를 많이 다룰 때 좋음.
    - 성능이 중요하면 최신 모델을 사용하고, 지연 시간이 중요하면 Gemini flash 를 사용한다.

개인정보

- 원친적으로 개인정보는 절대 외부 API로 나가면 안 된다.
- 개인정보를 다루는 팀에서 미리 만들어둔 MCP 서버를 호출한다. (데이터가 '마스킹' 된 상태로 넘어옴)